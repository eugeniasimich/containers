...
\begin{frame}{algunos tipos de datos}
  ...
  \end{block}
  \pause
  \Put(160,80){\includegraphics[scale=0.5]{img/bool}}
  \pause
  \begin{block}{\framebox{naturales}}
    \ExecuteMetaData[latex/Agda.tex]{nat}
  \end{block}
  \pause
  \Put(160,70){\includegraphics[scale=0.5]{img/nat}}
\end{frame}
...
\begin{frame}{más ejemplos}
  ...
  \begin{itemize}
    \item el container identidad representa al funtor identidad:\\\saltar
      ...
      \pause
      \ExecuteMetaData[latex/Container.tex]{cId}
      \pause
    ...
    \item el container constante ignora el argumento y siempre construye un tipo constante:\\
      ...
      \pause
      \ExecuteMetaData[latex/Container.tex]{cK}
      \pause
...
\begin{frame}{map de containers}
  \begin{itemize}
  \item notemos que si $C$ es un container, \AgdaFunction{$\llbracket$} $C$ \AgdaFunction{$\rrbracket$} : \AgdaDatatype{Set} $\to$ \AgdaDatatype{Set}
...
\begin{frame}[fragile]{ejemplo: cabeza de lista}
  la función \AgdaFunction{head} de listas retorna el primer elemento de una lista no vacía\\
  ¿cómo la expresamos en términos de los containers \AgdaFunction{cList} y \AgdaFunction{cMaybe}?\\
  \pause
  recordemos:
  \begin{columns}[T,onlytextwidth]
    \column{0.4\textwidth}
  ...
  en resumen,
    ...
    \item lo que guarda la única posición de la forma \AgdaInductiveConstructor{true} proviene de la primera posición de la lista
    \end{itemize}
\end{frame}
